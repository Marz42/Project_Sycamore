...
| 节点基础元数据 | Markdown Front Matter | `id`、`slug`、`title`、`domain`、`type`、`claimedLevel`。 |
...
每个 AbilityNode 属于以下四种类型之一，决定其 Markdown 结构灵魂与 AI 考核方式：

| 值 | 适用领域 | 核心关注点 | 考核方式 |
|:--|:--|:--|:--|
| `capability` | IT、软件操作、外语写作 | 步骤、坑点、Cheatsheet | 能否在给定场景下正确执行操作？ |
| `concept` | 哲学、历史、经济学、管理 | 核心主张、历史语境、批判 | 能否用该框架分析一个新的现实事件？ |
| `theorem` | 数学、物理、算法理论 | 公式、直觉推导、极值边界 | 边界条件失效时能否识别反例？ |
| `process` | 化工、机械、生物学机制 | 机理、参数妥协、物理安全 | 外部扰动时如何调节参数维持稳定？ |

每种类型的 Markdown 模板结构不同（见 Phase 2 路线图）。当前默认按 `capability` 处理。
...
| `nodeType` | enum | 是 | Markdown | 节点类型。默认 `capability`。见 NodeType 枚举。 |
...
| `type` | enum | 是 | 关系类型：`prerequisite` / `related` / `similar_pattern` / `contrasts_with` / `used_in_scenario` / `contrast` / `composition` / `diagnostic`。 |
...
syca promote <capture-id> --type concept --domain philosophy
syca promote <capture-id> --type theorem --domain math
```
...
- `--type` 指定节点类型（默认 `capability`），选择对应 Markdown 模板。
- 生成 Markdown 节点。
- 写入 Front Matter（含 `type` 字段）。
- 插入 ability_nodes 索引。
...
> **状态：计划中。** `--json` 参数尚未在 CLI 中实现。以下为设计契约，实现时以此为准。
