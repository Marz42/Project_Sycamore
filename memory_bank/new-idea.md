...
> **问题**：当前 `syca capture` 是一次性无状态命令。在沉浸式阅读（书本、文档、教程）场景下，频繁切终端执行单次命令会撕裂注意力——打字负担重、元数据重复指定、心流被打断。

---

## 方案一：CLI 状态化 —— "阅读心流模式"（Reading Session）

引入 **Session** 概念。开始阅读前设定上下文，进入 REPL 循环。

```bash
uv run syca session start --source "《人类简史》" --domain history --type concept
```

终端进入纯净循环界面：

```text
[Reading: 《人类简史》 | Domain: history]
(Press Ctrl+D to exit, Enter to capture)
>
```

每次回车自动将输入作为 capture 入库，**自动继承** Session 的 `source`、`domain`、`type`。

### 体验收益

| 对比 | 当前 | Session 模式 |
|:--|:--|:--|
| 录入 10 条笔记 | 10 次完整 CLI 命令 | 10 次回车 |
| 元数据 | 每次手动 `--domain` | 自动继承 |
| 注意力 | 频繁切终端 | 终端即草稿纸 |

---

## 方案二：降维接入 —— "草稿箱解析"（Draft Ingestion）

适用于双屏（电子书 + 编辑器）或 Obsidian/记事本用户。在纯文本文件里狂敲，最后一条命令导入。
...
- 状态函数：只和当前状态有关，和路径无关。比如内能。
- 焓（Enthalpy）：H = U + PV。工程计算方便，衡量等压过程热量。
- 熵增原理：孤立系统熵永远增加。解释覆水难收。
```

```bash
uv run syca inbox import scratch.md
```
...
- `# domain:` → `domain`
- `# type:` → `type`
- `- ` 开头的行 → 一条 capture（以空行分隔则支持多行正文）
- 导入后 `scratch.md` 可配置为自动清空

---

## 方案三：关联性自动编织 —— "顺序链预判"（Sequential Linking）

在 `syca promote` 环节加入 **同源感知（Source Awareness）**。

当连续 promote 来自同一 Session / 同一来源的节点时：

> *"检测到这属于同一阅读会话。要自动将前一个节点设为当前节点的 prerequisite 吗？ [Y/n]"*

### 体验收益

读完一本书清空 Inbox 后，`syca graph` 展示的不是一盘散沙，而是自动生成的**知识链条**。
...
1. **Inbox 表扩充**：`source`、`pre_domain`、`pre_type` 字段（或塞入 payload_json）
2. **`syca session` 命令**：`while True: input()` 循环 + 预设变量 INSERT
3. **`syca import` 命令**：读取文本，以 `- ` 或空行为分隔符切分入库
4. **Promote 同源感知**：对比当前与前一个 promote 的 source 字段
