...
| `id` / `slug` / `title` / `domain` | Markdown front matter | CLI 可修改，但必须写回 Markdown。 |
...
| 功能 | 简述 | 状态 |
|------|------|------|
| Capture | 低摩擦保存命令、笔记、链接和片段。 | 待开发 |
| Inbox | 查看、归档、丢弃捕获项。 | 待开发 |
| Promote | 将 CaptureItem 升格为 AbilityNode。 | 待开发 |
| Markdown 节点模板 | 生成标准能力节点文件。 | 待开发 |
| SQLite 本地索引 | 保存捕获项、节点索引、事件和搜索数据。 | 待开发 |
| Query Cheatsheet | 快速查询正式节点中的 Cheatsheet。 | 待开发 |
| Sync / Doctor | 检查 Markdown 与 SQLite 一致性。 | 待开发 |

## P1：能力校准

| 功能 | 简述 | 状态 |
|------|------|------|
| ReviewRun | 结构化 LLM 评审和 JSON 归档。 | 待开发 |
| CapabilityEvent | 用事件推导 freshness 和 readiness。 | 待开发 |
| Practice Log 工具 | 快速追加实践记录。 | 待开发 |
| 手动 Link | 支持弱关系与可选 rationale。 | 待开发 |
| Stale View | 显示需要恢复的能力节点。 | 待开发 |

## P2：能力地图与恢复训练

| 功能 | 简述 | 状态 |
|------|------|------|
| Recover Drill | 重新解释或按 Cheatsheet 完成任务。 | 待开发 |
| Graph View | 终端或 GUI 能力图。 | 待开发 |
| Asset 引用 | 管理 PDF、图片、链接等资料。 | 待开发 |
| 语义检索 | 在节点质量稳定后再评估。 | 暂缓 |
